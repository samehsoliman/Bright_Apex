...
--ahlam 05032025 
CREATE OR REPLACE FUNCTION STUFF_LABOR_FLAG(V_USER_NAME VARCHAR2)RETURN NUMBER IS
CURSOR C1 IS
    SELECT STUFF_LABOR_FLAG
    FROM USERS
    WHERE USER_NAME = V_USER_NAME;
V_STUFF_LABOR_FLAG NUMBER;
BEGIN
    V_STUFF_LABOR_FLAG :=1;
    OPEN C1;
    FETCH C1 INTO V_STUFF_LABOR_FLAG;
    CLOSE C1;
    /*
    IF V_STUFF_LABOR_FLAG = 1 THEN 
            RETURN(TRUE);
    ELSE
            RETURN(FALSE);
    END IF;
...
Insert into  USER_PARAMETERS
   (USER_PARAMETER_ID, BRANCH_ID, PARAMETER_NAME, PARAMETER_VALUE, SELECT_STATEMENT, SELECT_VALUE, SUB_SYSTEM_ID, VARIABLE1, VARIABLE2, ARABIC_PARAMETER_NAME)
 Values
   (1041, 1, 'Auto Coding For Employees', '2', 'SELECT DECODE(GET_USER_LANG(:GLOBAL.LANGUAGE_ID), ''P'', PRIMARY_NAME, SECONDARY_NAME) NAME, CODES_SYS_ID ID
FROM CODES_SYS
WHERE CODE_TYPES_ID = 26', 'SELECT DECODE(GET_USER_LANG(P1), ''P'', PRIMARY_NAME, SECONDARY_NAME) NAME
FROM CODES_SYS
WHERE CODE_TYPES_ID = 26
AND CODES_SYS_ID = P2', '9', 'CONTROL.LANGUAGE_ID', 'USER_PARAMETERS.PARAMETER_VALUE', 'Auto Coding For Employees');
Insert into  USER_PARAMETERS
   (USER_PARAMETER_ID, BRANCH_ID, PARAMETER_NAME, PARAMETER_VALUE, SELECT_STATEMENT, SELECT_VALUE, SUB_SYSTEM_ID, VARIABLE1, VARIABLE2, UPDATED_BY, UPDATED_DATE, UPDATED_MACHINE)
 Values
   (1042, 1, 'Using Dynamic HR Documents', '2', 'SELECT DECODE(GET_USER_LANG(:GLOBAL.LANGUAGE_ID), ''P'', PRIMARY_NAME, SECONDARY_NAME) NAME, CODES_SYS_ID ID
FROM CODES_SYS
WHERE CODE_TYPES_ID = 26', 'SELECT DECODE(GET_USER_LANG(P1), ''P'', PRIMARY_NAME, SECONDARY_NAME) NAME
FROM CODES_SYS
WHERE CODE_TYPES_ID = 26
AND CODES_SYS_ID = P2', '9', 'CONTROL.LANGUAGE_ID', 'USER_PARAMETERS.PARAMETER_VALUE', 'ADMIN', TO_DATE('03/26/2025 11:59:59', 'MM/DD/YYYY HH24:MI:SS'), 'AHLAM-PC');
COMMIT;


ALTER TABLE EMPLOYEES 
 ADD (DOC_FILE_NAME  VARCHAR2(255));

ALTER TABLE EMPLOYEES 
 ADD (DOC_MIME  VARCHAR2(255));
 
ALTER TABLE EMPLOYEES 
 ADD (DOC_LAST_UPDATE  DATE);

ALTER TABLE EMPLOYEES 
 ADD (DOC_CHAR_SET  VARCHAR2(255));
 
ALTER TABLE EMPLOYEES 
 ADD (DOC_AS_FILE BLOB);
    
ALTER TABLE EMPLOYEES 
  MOVE LOB (DOC_AS_FILE)
  STORE AS SECUREFILE;
  
  
ALTER TABLE EMPLOYEES_DOC_EXPIRY
 ADD (DOC_FILE_NAME  VARCHAR2(255));
...
-- ahlam 10-04-2025
CREATE OR REPLACE PACKAGE PCK_PAYROLL AS
/******************************************************************************  
   NAME:       PCK_PAYROLL  
   PURPOSE:   PACKAGE AS PLL IN FORMS ERP 
   CRETAED BY: AHLAM ABDULLAH GOMAH 
******************************************************************************/
  
    FUNCTION GET_DAY_PER_MONTH(V_DATE DATE , P_BRANCH_ID   NUMBER) RETURN NUMBER;
    FUNCTION GET_BASIC_SALARY_DAY_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE, P_BRANCH_ID   NUMBER) RETURN NUMBER ;
    FUNCTION GET_CALCULATION_METHOD(P_BRANCH_ID   NUMBER) RETURN NUMBER;
    FUNCTION GET_DAY_ABSENT_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE , P_BRANCH_ID   NUMBER) RETURN NUMBER;
    FUNCTION GET_DAY_LATE_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE ,P_BRANCH_ID   NUMBER) RETURN NUMBER ;
    FUNCTION GET_DAY_PENALTY_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE ,P_BRANCH_ID   NUMBER) RETURN NUMBER;
    FUNCTION GET_DAY_REWARD_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE ,P_BRANCH_ID   NUMBER) RETURN NUMBER ;
    FUNCTION GET_EMP_SCHEDULE(V_EMPLOYEE_ID NUMBER, V_DATE DATE , P_BRANCH_ID   NUMBER , P_LANGUAGE_ID NUMBER) RETURN NUMBER;
    FUNCTION GET_HOURS_PER_DAY(V_EMPLOYEE_ID NUMBER, P_BRANCH_ID   NUMBER) RETURN NUMBER;
    FUNCTION GET_OVERTIME_FACTOR(  P_BRANCH_ID   NUMBER) RETURN NUMBER;
    FUNCTION GET_OVERTIME_HOUR_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE, V_OVERTIME_FACTOR IN OUT NUMBER , P_BRANCH_ID   NUMBER ) RETURN NUMBER;
    FUNCTION GET_EMP_HOURS_PER_DAY(V_EMPLOYEE_ID NUMBER,P_HOURS_PER_DAY NUMBER,P_BRANCH_ID NUMBER) RETURN NUMBER;
    --PROCEDURE SET_EMPLOYEE_TYPE_LIST(V_ITEM_NAME VARCHAR2,NOT_REQ_FLAG NUMBER);

END PCK_PAYROLL;
/


CREATE OR REPLACE PACKAGE BODY PCK_PAYROLL IS
 
FUNCTION GET_DAY_PER_MONTH(V_DATE DATE , P_BRANCH_ID   NUMBER) RETURN NUMBER IS
CURSOR C1(V_BRANCH_ID NUMBER) IS
    SELECT DAYS_PER_MONTH  , CALCULATION_METHOD
    FROM BRANCH_SETUP
    WHERE BRANCH_ID = V_BRANCH_ID;
V_C1 C1%ROWTYPE;
BEGIN
    ...
    IF V_C1.CALCULATION_METHOD = 1 THEN
            RETURN(NVL(V_C1.DAYS_PER_MONTH,30));
    ELSE
            RETURN(SUBSTR(LAST_DAY(V_DATE),1,2));
    END IF;
END;


FUNCTION GET_CALCULATION_METHOD (P_BRANCH_ID   NUMBER) RETURN NUMBER IS
CURSOR C1(V_BRANCH_ID NUMBER) IS
    SELECT CALCULATION_METHOD
    FROM BRANCH_SETUP
    WHERE BRANCH_ID = V_BRANCH_ID;
V_CALCULATION_METHOD NUMBER;
BEGIN
    OPEN C1(P_BRANCH_ID);
    FETCH C1 INTO V_CALCULATION_METHOD;
    CLOSE C1;
    RETURN(V_CALCULATION_METHOD);
END;


FUNCTION GET_DAY_ABSENT_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE , P_BRANCH_ID   NUMBER) RETURN NUMBER IS
CURSOR C1(V_SAL_PAYMENT  NUMBER)  IS
    SELECT SUM(AMOUNT * DECODE(SUBSTR(ENTITLE_DEDUCT_TYPE, 1,1),1,1,-1))
    FROM   EMPLOYEES_ENTITLE_DEDUCT_VIEW
    WHERE  EMPLOYEE_ID = V_EMPLOYEE_ID
    AND    TO_CHAR(SAL_MONTH ,'MM-RRRR') = TO_CHAR(V_DATE,'MM-RRRR')
    AND    PAYMENT_NUMBER = V_SAL_PAYMENT
    AND    SUBJECT_TO_ABSENT = 1;
CURSOR C2(V_BRANCH_ID NUMBER) IS
    SELECT ABSENT_DAY_FACTOR
    FROM BRANCH_SETUP
    WHERE BRANCH_ID = V_BRANCH_ID;
V_DAY_PER_MONTH NUMBER := 30;
V_HOURS_PER_DAY NUMBER := 8;
V_AMOUNT NUMBER;
V_VALUE NUMBER;
V_ABSENT_DAY_FACTOR NUMBER :=1;
V_SAL_PAYMENT  NUMBER;
BEGIN
    V_DAY_PER_MONTH := GET_DAY_PER_MONTH(V_DATE,P_BRANCH_ID);
    SELECT PAYMENT_NUMBER 
    INTO   V_SAL_PAYMENT
    FROM   PAY_PAYMENT
    WHERE  SALARY_PAYMENT  = 1 
    AND    BRANCH_ID = P_BRANCH_ID;
    
    OPEN C1(V_SAL_PAYMENT);
    FETCH C1 INTO V_AMOUNT;
    CLOSE C1;
    OPEN C2(P_BRANCH_ID);
    FETCH C2 INTO V_ABSENT_DAY_FACTOR;
    CLOSE C2;
    RETURN((V_AMOUNT / V_DAY_PER_MONTH) * NVL(V_ABSENT_DAY_FACTOR,1));
END;
    
    
FUNCTION GET_DAY_LATE_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE ,P_BRANCH_ID   NUMBER) RETURN NUMBER IS
CURSOR C1 (V_SAL_PAYMENT NUMBER)IS
    SELECT SUM(AMOUNT * DECODE(SUBSTR(ENTITLE_DEDUCT_TYPE, 1,1),1,1,-1))
    FROM   EMPLOYEES_ENTITLE_DEDUCT_VIEW
    WHERE  EMPLOYEE_ID = V_EMPLOYEE_ID
...
BEGIN
    ...
    FETCH C1 INTO V_AMOUNT;
    CLOSE C1;
    RETURN(V_AMOUNT / V_DAY_PER_MONTH);
    
END;


FUNCTION GET_DAY_PENALTY_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE ,P_BRANCH_ID   NUMBER) RETURN NUMBER IS
CURSOR C1 (V_SAL_PAYMENT NUMBER)IS
    SELECT SUM(AMOUNT * DECODE(SUBSTR(ENTITLE_DEDUCT_TYPE, 1,1),1,1,-1))
    FROM   EMPLOYEES_ENTITLE_DEDUCT_VIEW
    WHERE  EMPLOYEE_ID = V_EMPLOYEE_ID
...
FUNCTION GET_DAY_REWARD_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE ,P_BRANCH_ID   NUMBER) RETURN NUMBER IS
CURSOR C1 (V_SAL_PAYMENT NUMBER)IS
    SELECT SUM(AMOUNT * DECODE(SUBSTR(ENTITLE_DEDUCT_TYPE, 1,1),1,1,-1))
    FROM   EMPLOYEES_ENTITLE_DEDUCT_VIEW
    WHERE  EMPLOYEE_ID = V_EMPLOYEE_ID
...
BEGIN
    ...
    CLOSE C1;
    RETURN(V_AMOUNT / V_DAY_PER_MONTH);
    --RETURN(ROUND(V_AMOUNT / (V_DAY_PER_MONTH), TO_NUMBER(NAME_IN('GLOBAL.ROUND_OF_DECIMAL'))));
    
END;


FUNCTION GET_EMP_SCHEDULE(V_EMPLOYEE_ID NUMBER, V_DATE DATE , P_BRANCH_ID   NUMBER , P_LANGUAGE_ID NUMBER) RETURN NUMBER IS
CURSOR C1_NEW IS
    SELECT SCHEDULE_DEF_ID
    FROM EMP_SCHEDULE_MODIFY
    WHERE EMPLOYEE_ID = V_EMPLOYEE_ID
...
BEGIN
    ...
    IF V_EMPLOYEE_ID IS NOT NULL AND V_DATE IS NOT NULL THEN
            ...
            OPEN C1_NEW;
            FETCH C1_NEW INTO V_SCHEDULE_DEF_ID;
            CLOSE C1_NEW;
            IF V_SCHEDULE_DEF_ID IS NULL THEN
                OPEN C1;
                FETCH C1 INTO V_SCHEDULE_DEF_ID;
                CLOSE C1;
                IF V_SCHEDULE_DEF_ID IS NULL THEN
                        OPEN C2(P_BRANCH_ID);
                        FETCH C2 INTO V_SCHEDULE_DEF_ID;
                        CLOSE C2;
                END IF;
            END IF;
            ...
            IF V_SCHEDULE_DEF_ID IS NULL THEN
                    ...
                    SELECT USER_CODE, DECODE(GET_USER_LANG(V_LANGUAGE_ID), 'P', PRIMARY_NAME, SECONDARY_NAME) NAME 
                    INTO V_EMPLOYEE_CODE, V_EMPLOYEE_NAME
                    FROM EMPLOYEES
                    WHERE EMPLOYEES.EMPLOYEE_ID = V_EMPLOYEE_ID;
                    
                    apex_error.add_error(
                                            p_message          => PCK_BRIGHT.GET_ERROR_MESSAGE_LANG('10000155' ,'E', '('||V_EMPLOYEE_CODE||','||V_EMPLOYEE_NAME||','||V_DATE||')' , V_LANGUAGE_ID),
                                            p_additional_info  => null,
                                            p_display_location => apex_error.c_inline_in_notification
                                        );
            END IF;
    END IF;
    RETURN(V_SCHEDULE_DEF_ID);
END;


FUNCTION GET_HOURS_PER_DAY(V_EMPLOYEE_ID NUMBER, P_BRANCH_ID   NUMBER) RETURN NUMBER IS
CURSOR C1 IS
...
CURSOR C2(V_BRANCH_ID NUMBER) IS
    ...
    FROM BRANCH_SETUP
    WHERE BRANCH_ID = V_BRANCH_ID;
V_HOURS_PER_DAY NUMBER :=NULL;
BEGIN
    OPEN C1;
    FETCH C1 INTO V_HOURS_PER_DAY;
    CLOSE C1;
    IF V_HOURS_PER_DAY IS NULL THEN
            OPEN C2(P_BRANCH_ID);
            FETCH C2 INTO V_HOURS_PER_DAY;
            CLOSE C2;
    END IF;
    RETURN(V_HOURS_PER_DAY);
END;

    
FUNCTION GET_OVERTIME_FACTOR (  P_BRANCH_ID   NUMBER) RETURN NUMBER IS
CURSOR C1(V_BRANCH_ID NUMBER) IS
    SELECT OVERTIME_FACTOR
    FROM BRANCH_SETUP
    WHERE BRANCH_ID = V_BRANCH_ID;
V_OVERTIME_FACTOR NUMBER;
BEGIN
    OPEN C1(P_BRANCH_ID);
    FETCH C1 INTO V_OVERTIME_FACTOR;
    CLOSE C1;
    RETURN(V_OVERTIME_FACTOR);
END;


FUNCTION GET_OVERTIME_HOUR_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE, V_OVERTIME_FACTOR IN OUT NUMBER , P_BRANCH_ID   NUMBER ) RETURN NUMBER IS
CURSOR C1(V_SAL_PAYMENT NUMBER) IS
    SELECT SUM(AMOUNT * DECODE(SUBSTR(ENTITLE_DEDUCT_TYPE, 1,1),1,1,-1))
    FROM   EMPLOYEES_ENTITLE_DEDUCT_VIEW
    WHERE  EMPLOYEE_ID = V_EMPLOYEE_ID
    AND    TO_CHAR(SAL_MONTH ,'MM-RRRR') = TO_CHAR(V_DATE,'MM-RRRR')
    AND    PAYMENT_NUMBER = V_SAL_PAYMENT
    AND    SUBJECT_TO_OVERTIME = 1;
V_DAY_PER_MONTH NUMBER := 30;
V_HOURS_PER_DAY NUMBER := 8;
V_AMOUNT NUMBER;
CURSOR C2 IS
    SELECT FIXED_OVERTIME_VALUE
    FROM EMPLOYEES
    WHERE EMPLOYEE_ID = V_EMPLOYEE_ID;
V_FIXED_OVERTIME_VALUE NUMBER :=0;
V_SAL_PAYMENT NUMBER ;
BEGIN    
    SELECT PAYMENT_NUMBER 
    INTO   V_SAL_PAYMENT
    ...
    OPEN C2;
    FETCH C2 INTO V_FIXED_OVERTIME_VALUE;
    CLOSE C2;
    IF V_FIXED_OVERTIME_VALUE IS NOT NULL AND V_FIXED_OVERTIME_VALUE > 0 THEN
            RETURN(V_FIXED_OVERTIME_VALUE);
    END IF;
    
    V_DAY_PER_MONTH := GET_DAY_PER_MONTH(V_DATE,P_BRANCH_ID);
    V_HOURS_PER_DAY := GET_HOURS_PER_DAY(V_EMPLOYEE_ID,P_BRANCH_ID);
    
    IF V_OVERTIME_FACTOR IS NULL THEN
            V_OVERTIME_FACTOR := GET_OVERTIME_FACTOR(P_BRANCH_ID);
    END IF;
    
    OPEN C1(V_SAL_PAYMENT);
    FETCH C1 INTO V_AMOUNT;
    CLOSE C1;
    RETURN(V_AMOUNT / (V_DAY_PER_MONTH * V_HOURS_PER_DAY)* V_OVERTIME_FACTOR);
END;




FUNCTION GET_BASIC_SALARY_DAY_VALUE(V_EMPLOYEE_ID NUMBER, V_DATE DATE,P_BRANCH_ID NUMBER) RETURN NUMBER IS
CURSOR C1 IS
    SELECT SUM(AMOUNT)
    FROM EMPLOYEES_ENTITLE_DEDUCT_VIEW
    WHERE EMPLOYEE_ID = V_EMPLOYEE_ID
    AND ENTITLE_DEDUCT_TYPE = 11;
V_DAY_PER_MONTH NUMBER := 30;
V_AMOUNT NUMBER;
BEGIN
    V_DAY_PER_MONTH := GET_DAY_PER_MONTH(V_DATE,P_BRANCH_ID);
...
FUNCTION GET_EMP_HOURS_PER_DAY(V_EMPLOYEE_ID NUMBER,P_HOURS_PER_DAY NUMBER,P_BRANCH_ID NUMBER) RETURN NUMBER IS
CURSOR C1 IS
	SELECT HOURS_PER_DAY
	FROM BRANCH_SETUP
...
BEGIN
	IF P_HOURS_PER_DAY IS NOT NULL THEN
			RETURN(P_HOURS_PER_DAY);
	ELSE
			OPEN C1;
			FETCH C1 INTO V_HOURS_PER_DAY;
			CLOSE C1;
